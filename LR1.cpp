/*
  ...
  Author: 娄天宇
  Date: 07-06-07
　Description:对任意给定的文法G构造LR(1)项目集规范族和分析表,并对输入的句子进行语法分析
*/
#include"iostream"
#include"fstream"
#include"stack"
...
char buffer[50];            //用来存放生成CLOSURE(I)时需要的first_set 也用来读入用户的输入串
int  bsize = 0;
...
void Read_G()
{
    ...
    for (int i = 91; i < 128; i++)
        if (tempofinput[i])
            str_vt[size_vt++] = i;
    for (int i = 65; i < 91; i++)  //65-91是大写英文字母，作为终结符
        if (tempofinput[i])  
            str_vn[size_vn++] = i;
}  
...
对每一个文法符号X计算FIRST(X)
若X∈VT, FIRST(X)={X}
若X∈VN, FIRST(X)={a|X→a…,a∈VT}
若X∈VN, 且有产生式X →ε，则{ε} ∈ FIRST(X)
若X∈VN, 且有产生式X →Y1Y2…Yn，且Y1Y2…Yn∈ VN
当Y1 ，Y2 ， … ， Yi-1 ⇒ ε ，则FIRST(Y1)-{ε}, FIRST(Y2)-{ε}… FIRST(Yi-1)-{ε}, FIRST(Yi)都包含在FIRST(X)中
当Yi ⇒ ε(i=1,2…n),将{ε}并入FIRST(X)中
*/
void get_first() {  //计算First集合
    bool flag1;
    do {
        flag1 = false;
        for (int i = 1; i <= number; i++) {
            ...
            do {
                flag2 = false;
                if (G[i][t] >= 'A' && G[i][t] <= 'Z') { //计算非终结符的first元素集
                    for (int k = 0; k < 64; k++)
...
void  gete_expc(proj temp) {   //计算LR1的预测项
    bsize = 0;
    bool flag;
    int tt = temp.part;
    do {
        ...
        if (tt + 1 >= length[temp.formula_numb]) {
        ...
        else if (G[temp.formula_numb][tt + 1] < 'A' || G[temp.formula_numb][tt + 1] > 'Z') {  //非终结符直接加
            buffer[bsize++] = G[temp.formula_numb][tt + 1];
            return;
        }

        else if (G[temp.formula_numb][tt + 1] >= 'A' && G[temp.formula_numb][tt + 1] <= 'Z') {  //终结符找first_vn
            for (int i = 0; i < 64; i++) {
            ...
            if (first_vn[G[temp.formula_numb][tt + 1] - 'A'][64]) {  //ASCII64 是@，在本程序中@代表空项ε
                tt++;
                flag = true;
            }
        }
...
int main() {
    ...
    G_ifile.open("grammar.txt"); //Read grammar
    input_ifile.open("input.txt"); //Read input text to be parsed
    items_ofile.open("items.txt"); //Output LR1 Items
    act_ofile.open("action_table.txt"); //Output LR1 Action Table


    Read_G();        //read G and put the number of formula into count
    get_first();     //get each vn's first_set
    go();   //get GO()
    ...
    do {
        ...
        for (int i = 0; i < size_act_table[work_sta]; i++)
            if (action_table[work_sta][i].ch == buffer[index_buf]) {
                ...
                else if (action_table[work_sta][i].nxt_sta > 0) {  //Shift
